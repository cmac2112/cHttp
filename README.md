...
`hints` parameter points to a struct addrinfo that you've already filled out with relevant information that you care about
we zero out the rest with memset and the funcion returns results matching your criteria

```c++

memset(&hints, 0, sizeof hints);   // zero everything out first
  hints.ai_family = AF_UNSPEC;       // accept IPv4 or IPv6
  hints.ai_socktype = SOCK_STREAM;   // only TCP (not UDP)
```
